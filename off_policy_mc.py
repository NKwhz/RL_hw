...
class Agent:
    def __init__(self):
        ...
        self.expolicy = np.full((state_size, action_size), 0.25)
        self.c = np.zeros((state_size, action_size))
    ...
    def train(self, first=True):
        for i in range(Iter):
            ...
            for j in range(len(self.states) - 1, -1, -1):
                ...
                a = self.actions[j]
                self.c[s, a] += w
                self.g[s, a] += w / self.c[s, a] * (g - self.g[s, a])
